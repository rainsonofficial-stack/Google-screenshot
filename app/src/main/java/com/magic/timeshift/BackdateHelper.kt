...
object BackdateHelper {

    fun resolveTargetTime(context: Context): Long {
        val prefs = context.getSharedPreferences("timeshift_prefs", Context.MODE_PRIVATE)
        ...
        return when (prefs.getString("time_setting", "3h")) {
            ...
            "10h" -> {
                ...
                now.add(Calendar.MINUTE, -13)
                now.timeInMillis
            }
            "24h" -> {
                now.add(Calendar.HOUR_OF_DAY, -25)
                now.add(Calendar.MINUTE, -26)
                now.timeInMillis
            }
            "3d" -> {
    ...
    fun saveBackdatedScreenshot(context: Context, bitmap: Bitmap, timeMillis: Long) {
        ...
        try {
            ...
            // MediaStore date fields above still apply
        }
